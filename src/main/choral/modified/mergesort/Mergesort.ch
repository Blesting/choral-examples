package choral.modified.Mergesort;
...
public class Mergesort@( A, B, C ){
	...
	public List@A< Integer > sort ( List@A< Integer > a ){
		if( a.size() > 1@A ){
			ch_AB.< MChoice >select( MChoice@A.L ); 
			ch_CA.< MChoice >select( MChoice@A.L );
			...
			ch_AB.< MChoice >select( MChoice@A.R );	
			ch_CA.< MChoice >select( MChoice@A.R );
	...
	private List@A< Integer > merge ( List@B< Integer> lhs, List@C< Integer> rhs ) {
		if( lhs.size() > 0@B ) {
			...
			if( rhs.size() > 0@C ){
				...
				ch_BC.< MChoice >select( MChoice@C.R );
				ch_AB.< MergeA >select( MergeA@B.THREE );
